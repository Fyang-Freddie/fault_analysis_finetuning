...
你要生成的是“事实依据驱动的工程推理 QA”，不是“方法论 QA”“流程性 QA”“阅读理解 QA”或“空泛建议 QA”。

每个 QA 都必须训练模型完成以下能力：

1. 从故障现象中识别异常特征。
2. 从宏观检查、化学成分、金相检验、硬度试验、微观分析、运行参数等内容中提取关键证据。
3. 根据单项或多项检测证据判断可能的失效机理。
4. 根据检测证据排除不符合事实的失效原因。
5. 在证据充分时给出失效结论或倾向性判断。
6. 在证据不足时明确说明不能直接下结论，并提出进一步检测建议。
7. 根据失效机理、风险证据或已明确结论提出工程整改、预防或运行维护建议。

你生成的 QA 应重点体现以下推理链条：

“发现了什么事实情况 → 这些事实说明什么工程问题 → 多项证据如何相互印证 → 可以排除哪些不符合证据的原因 → 能支持什么失效结论或风险判断 → 应提出什么工程建议或补充检测建议”。

每个 QA 必须围绕输入内容中已经出现的具体事实证据展开，例如：
...
* 裂纹起源、扩展路径、裂纹形貌、腐蚀坑、断口形貌、微区元素、运行异常等事实共同指向某种失效机理。
* 可以判断为应力腐蚀开裂、疲劳、腐蚀减薄、冲刷腐蚀、磨损、过载、热疲劳、振动疲劳、润滑失效、密封失效、焊接缺陷等。
...
* 宏观检查、金相、硬度、化学成分、SEM/EDS、运行工况等多项证据共同支持某个结论。
* 多项正常结果与异常结果共同限定失效原因。
...
* 输入中出现腐蚀坑、裂纹、壁厚减薄、泄漏、振动升高、温度异常、磨损加剧、疲劳裂纹扩展等风险事实。
* 但证据尚不足以完全确定最终失效机理。
...
* 输入中出现“化学成分符合标准、硬度正常、基体组织正常、未见明显制造缺陷、断口无一次性过载特征”等事实。
* 应说明这些事实为什么不支持某些失效原因。
...
* 输入中已经给出明确或较充分支持的失效机理、风险来源或整改依据。
* 建议必须对应证据，不能泛泛写“加强管理、定期检查”。
...
* 输入中只有裂纹、腐蚀、泄漏、断裂、磨损等现象，但缺少金相、硬度、断口、成分、腐蚀产物、运行载荷等关键证据。
* answer 必须明确“现有证据不足以直接判断最终失效机理”。
...
6. 如果证据不足以支持最终失效机理判断，只能生成“异常现象 → 风险判断型”或“证据不足 → 补充检测型”QA，不得强行下结论。
7. 如果输入中既有异常证据，也有正常证据，应优先生成“证据组合 → 结论推导型”或“检测证据 → 排除原因型”QA。
8. 如果输入中已经明确给出失效结论，应优先生成“事实发现 → 机理判断型”“证据组合 → 结论推导型”或“结论 → 工程建议型”QA。

核心事实约束：

1. 必须严格基于输入内容生成问答，不得编造未出现的设备、材料、检测结果、工况条件、失效机理或工程结论。
2. 如果输入中没有失效结论，但检测事实足够典型，可以使用“倾向于”“可能支持”“优先考虑”等表述，不得写成绝对结论。
3. 如果输入中没有足够证据，不得伪造最终失效机理，应明确证据不足。
4. 如果输入中只有检测现象，优先生成风险判断或补充检测建议型 QA。
5. 如果输入中明确给出结论，可以生成机理判断型 QA，但 thinking 必须展示支持该结论的事实证据。
6. 如果输入中出现“成分符合标准、组织正常、硬度正常、未见明显制造缺陷”等信息，优先生成排除原因型 QA。
7. 如果输入中出现“裂纹从内壁起裂、外壁起裂、穿晶、沿晶、分叉、树枝状、点蚀、腐蚀产物、疲劳条带、韧窝、解理、磨损沟槽、冲刷痕迹”等信息，优先生成机理判断、证据组合或风险判断 QA。
8. 如果输入中出现运行参数异常，例如振动、温度、压力、流量、泄漏、噪声、润滑油污染等，应优先生成故障链条、风险判断或工程建议型 QA。
9. 如果证据不足以形成完整故障链条，不要强行补充缺失环节。
...
只有当检测方法和具体检测结果同时出现时，才可以生成“检测证据解释或结论推导型 QA”。

允许的问题形式：

* “UT 发现某管段存在局部减薄，而外观检查发现内壁冲刷痕迹时，这些证据说明什么工程问题？”
* “SEM 观察到疲劳条带，且裂纹源位于应力集中区域时，为什么支持疲劳开裂判断？”
* “EDS 检出氯元素富集，同时裂纹呈分叉状穿晶扩展时，对应力腐蚀开裂判断有什么帮助？”
* “化学成分和硬度均符合要求，但裂纹从腐蚀坑处萌生时，为什么不宜将失效主因归结为材料错用？”
* “现有资料只有裂纹和腐蚀痕迹时，为什么不能直接判断为应力腐蚀开裂，还需要补充哪些检测？”
...
7. thinking 应尽量体现以下结构：

   * 发现了什么事实情况；
   * 该事实说明什么工程问题；
   * 多项证据之间如何相互印证；
   * 哪些原因可以排除，哪些原因还不能排除；
   * 能支持什么失效机理、风险判断、工程建议或补充检测建议。
...
5. 如果证据不足，answer 必须明确说明“不能直接下最终失效结论”。
...
"4. 多项证据之间的关联关系。",
"5. 可排除或不能排除的原因。",
"6. 可支持的失效机理、风险判断、工程建议或补充检测建议。"
],
"answer": "简洁明确的中文答案。",
"qa_type": "事实发现-机理判断型/证据组合-结论推导型/异常现象-风险判断型/检测证据-排除原因型/结论-工程建议型/证据不足-补充检测型"
}}
]
"""
...
请优先生成“事实依据驱动的工程推理 QA”，每个 QA 都必须围绕片段中已经出现的事实证据展开，体现：
...
围绕“检测发现了什么异常现象，这些异常现象说明可能发生了什么失效机理”。
...
当片段中已有明确失效原因、风险证据或整改依据时，生成针对性工程建议。

建议必须直接对应证据，例如：

* 腐蚀诱发开裂 → 介质控制、氯离子控制、防腐保护、腐蚀监测；
* 疲劳开裂 → 降低交变载荷、消除应力集中、优化支撑、振动监测；
* 冲刷腐蚀 → 控制流速、优化流道、检查壁厚、改善材料或防护层；
* 润滑失效 → 改善润滑油清洁度、过滤、油品监测、轴承状态监测；
* 密封失效 → 检查密封面、装配偏差、介质兼容性和运行压力波动。

禁止写没有证据支撑的“加强管理、定期检查、提高可靠性”等空泛建议。

6. 证据不足-补充检测型

当现有资料不足以判断最终失效机理时，必须生成补充检测建议型 QA。

适合场景：

* 只有裂纹现象，缺少断口分析；
* 只有腐蚀痕迹，缺少腐蚀产物或介质分析；
* 只有泄漏现象，缺少壁厚、组织、硬度、成分或断口证据；
* 只有磨损现象，缺少润滑、载荷、装配、振动数据；
* 只有宏观损伤，缺少微观分析。
...
5. 与输入事实无关的空泛工程管理问题。
6. 文章特指型阅读理解问题。
7. 只问“本文/该文档/该实验/该样品/上述案例”中有什么内容的问题。

特别限制：

* 不要生成“PT/RT/UT/MT/金相/硬度/SEM/EDS 有什么作用”这类纯方法问题。
* 只有当片段中同时出现“检测方法 + 具体检测结果”时，才可以生成检测证据解释或结论推导型 QA。
* 不要问“应该如何开展分析”“如何处理多个样品”“如何保证代表性”等方法论问题。
* 不要生成“该文档/该论文/该实验/该样品/上述案例/文中”等特指型问题。
* 如果片段中只有检测现象，没有最终结论，不要强行写最终失效机理。
* 如果片段中明确给出结论，可以生成机理判断型、结论推导型或工程建议型 QA，但 thinking 必须展示支持该结论的事实证据。
* 如果片段中出现“成分符合标准、组织正常、硬度正常、未见明显制造缺陷”等信息，优先生成排除原因型 QA。
* answer 通常控制在 1-4 句话。
* thinking 使用 JSON 数组，每个元素是一条编号证据链步骤。
* thinking 通常写 4-8 条；证据较少时可以少于 4 条，但不得硬凑。
* thinking 不要复述整段原文，只提炼关键事实、工程含义、证据关联、排除过程和结论依据。
...
* “已知材料成分合格、组织正常、内壁存在点蚀坑、裂纹从内壁起裂且呈树枝状扩展，应如何判断失效机理？”
...
* “如果只发现裂纹和腐蚀痕迹，但缺少金相、硬度、断口和腐蚀产物分析，为什么不能直接判断为应力腐蚀开裂？”
...
你的审核目标是：只保留“事实依据驱动的工程推理 QA”，删除“方法论 QA”“空泛建议 QA”“阅读理解 QA”“无依据推理 QA”和“证据不足却强行下结论的 QA”。

高质量 QA 必须训练模型完成以下能力：

1. 从故障现象中识别异常特征。
2. 从宏观检查、化学成分、金相检验、硬度试验、微观分析、运行参数等内容中提取关键证据。
3. 根据单项或多项检测证据判断可能失效机理。
4. 根据检测证据排除不符合事实的原因。
5. 在证据充分时给出失效结论或倾向性判断。
6. 在证据不足时说明不能直接下结论，并提出进一步检测建议。
7. 根据失效机理、风险证据或明确结论提出工程整改、预防或运行维护建议。
...
4. question 是否体现以下链条中的至少一部分：

   “发现了什么事实情况 → 说明什么工程问题 → 支持什么失效结论或风险判断 → 应提出什么建议或补充检测”。

5. thinking 是否是公开可展示的证据链，而不是模型自我思考。
6. thinking 中每一步是否都有事实依据或清晰的工程含义。
7. answer 是否简洁、准确、工程化。
8. 是否存在证据不足却强行判断为疲劳、SCC、腐蚀、过载、制造缺陷等问题。
9. 是否存在重复、空泛、低价值问题。
10. 是否已经从具体论文、实验、样品或设备编号中抽象为通用失效分析知识。
11. question 是否避免了“本文、该文档、该论文、该研究、该实验、该样品、该压力容器、该泵、该阀门、上述案例、文中”等文章特指表达。
12. qa_type 是否属于以下 6 类之一：
...
2. 空泛建议型 QA：
   ...
   * 没有具体失效原因或风险证据，却提出“加强管理、定期检查、优化维护、提高可靠性”等泛泛建议。

3. 无依据机理判断型 QA：

   * 原始片段没有足够证据，却强行判断为疲劳、SCC、腐蚀、过载、制造缺陷等。
   * 片段只出现裂纹或腐蚀痕迹，却直接写成明确最终失效机理。

4. 文章特指型 QA：

   * 问题依赖“本文、该文档、该样品、上述案例”等指代。
   * 问题只是在问某篇文章写了什么，而不是抽象成通用工程判断能力。

5. 阅读理解型 QA：

   * 只询问原文记录了哪些检测、用了哪些方法、得到了哪些数据，没有工程推理价值。
...
1. 根据具体异常现象判断可能失效机理的 QA。
2. 综合多项检测证据推导失效原因或结论的 QA。
3. 根据正常检测结果排除材料错用、热处理异常、制造缺陷、单纯过载等原因的 QA。
4. 根据腐蚀、裂纹、减薄、磨损、振动、温升等事实判断运行风险的 QA。
5. 根据明确失效机理或风险证据提出针对性工程建议的 QA。
6. 在证据不足时，只做风险提示或提出补充检测建议的 QA。

对证据不足型 QA 的特殊要求：

1. 如果原始片段证据不足，answer 必须明确说明“不能直接判断最终失效机理”。
2. 补充检测建议必须具体，例如：
